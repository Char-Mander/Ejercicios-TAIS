...
//Se han tocado las funciones inserta, rotaDer, rotaIzq
...
public:
	...
	//Funcion ejercicio 4
	void kesimo(int num, int & sol) {
		 kesimo(this->raiz, num, sol);
	}
...
protected:
	//Esta funcion tiene un coste logaritmico respecto al numero de elementos del arbol O(log n)
	void kesimo(Link a, int num, int & sol){
		if (a != nullptr) {
			if (num == a->tam_i) {
				sol = a->cv.clave;
			}
