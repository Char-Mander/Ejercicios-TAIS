...
//Hay añadidas dos funciones, rango(int e) y rango (Link a, int e). 
//Ambas tienen coste logarítmico, pues van podando el árbol dependiendo 
//de si el elemento que tenemos es mayor o menor que la raíz
...
public:
	...
	//El vector estaba como const
	void rango(int k1, int k2, std::vector<int> &v){
		 rango(this->raiz, k1, k2, v);
	}
...
protected:
	//Esta funcion tiene un coste lineal respecto al numero de elementos del vector O(n)
	void rango(Link a, int k1, int k2, std::vector<int> &v){
		if (a != nullptr) {
			if (menor(k1, a->cv.clave) && menor(a->cv.clave, k2)) {
				rango(a->iz, k1, k2, v);
				v.push_back(a->cv.clave);
				rango(a->dr, k1, k2, v);
			}
			else if (k1 == a->cv.clave) {
				v.push_back(a->cv.clave);
				rango(a->dr, k1, k2, v);
			}
			//Si la raiz es menor, sus hijos derechos pueden no serlo
			else if (menor(a->cv.clave, k1)) {
				rango(a->dr, k1, k2, v);
			}
			//Si es igual a k2 tambien cuenta
			else if (k2 == a->cv.clave) {
				rango(a->iz, k1, k2, v);
				v.push_back(a->cv.clave);
			}
			//Si es mayor que k2, sus hijos izquierdos no tienen por que serlo
			else if (menor(k2, a->cv.clave)) {
				rango(a->iz, k1, k2, v);
			}

		}
		//Con la excepcion al llegar a una hoja paraba la ejecucion
